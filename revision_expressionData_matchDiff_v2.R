...
# Rscript revision_expressionData_matchDiff_v2.R
...
outFolder <- file.path("REVISION_EXPRESSIONDATA_MATCHDIFF_V2_CORRECTED_V2")
dir.create(outFolder, recursive = TRUE)
...
matching_withRank_dt$tumor_minus_norm_cellline_mRNA <- matching_withRank_dt[,paste0("tumor_", col_var)] -  
  matching_withRank_dt[,paste0("norm_", col_var)] 


matching_withRank_dt$norm_over_tumor_cellline_mRNA <- matching_withRank_dt[,paste0("norm_", col_var)] /
  matching_withRank_dt[,paste0("tumor_", col_var)] 

matching_withRank_dt$tumor_over_norm_cellline_mRNA <- matching_withRank_dt[,paste0("tumor_", col_var)] /
  matching_withRank_dt[,paste0("norm_", col_var)] 
...
init_matching_withRank_dt <- matching_withRank_dt


all_exprds <- unique(matching_withRank_dt$ref_exprds)

for(myexprds in c("all", all_exprds)) {
  
  
  if(myexprds == "all") {
    matching_withRank_dt <- init_matching_withRank_dt
  } else {
    matching_withRank_dt <- init_matching_withRank_dt[init_matching_withRank_dt$ref_exprds == myexprds,]
    stopifnot(nrow(matching_withRank_dt) > 0)
  }
  ...
  plotTit <- paste0("FC from exprds vs. mRNA FPKM diff - ", myexprds)
  
  
  myx <- matching_withRank_dt[,paste0("ref_region_logFC")]  
  # myy <- log10(matching_withRank_dt$norm_minus_tumor_cellline_mRNA)
  myy <- log10(matching_withRank_dt$tumor_minus_norm_cellline_mRNA)
  
  toKeep <- !is.na(myx) & !is.na(myy)
  myx <- myx[toKeep]
  myy <- myy[toKeep]
  toKeep <- !is.infinite(myx) & !is.infinite(myy)
  myx <- myx[toKeep]
  myy <- myy[toKeep]
  
  
  outFile  <- file.path(outFolder, paste0("exprdsLogFC", "_vs_cellLinesFPKMdiff_",myexprds, "_densplot.", plotType))
  ...
  densplot(
    ...
    ylab=paste0("tumor-norm mRNA FPKM [log10]"),
    cex.main=plotCex,
    cex.axis=plotCex,
    cex.lab=plotCex,
    main=plotTit
  ...
  save(matching_withRank_dt, file="matching_withRank_dt_tmp.Rdata", version=2)
  
  myx <- matching_withRank_dt[,paste0("ref_region_logFC")]  
  # myy <- log2(matching_withRank_dt$norm_over_tumor_cellline_mRNA )
  myy <- log2(matching_withRank_dt$tumor_over_norm_cellline_mRNA )
  toKeep <- !is.na(myx) & !is.na(myy)
  myx <- myx[toKeep]
  myy <- myy[toKeep]
  toKeep <- !is.infinite(myx) & !is.infinite(myy)
  ...
  plotTit <- paste0("FC from exprds vs. mRNA FPKM ratio - ", myexprds)
  
  
  outFile  <- file.path(outFolder, paste0("exprdsLogFC", "_vs_cellLinesFPKMratio_", myexprds, "_densplot.", plotType))
  do.call(plotType, list(outFile, height=myWidth, width=myWidth))
  densplot(
    x= myx,
    y = myy,
    ...
    ylab=paste0("tumor/norm mRNA FPKM [log2]"),
    cex.main=plotCex,
    cex.axis=plotCex,
    cex.lab=plotCex,
    main=plotTit
  ...
  mtext(side=3, text=mySub)
  addCorr(x=myx,y=myy, bty="n", legPos="topright", corMet="spearman")
  abline(lm(myy~myx), col="darkgrey")
  foo <- dev.off()
  cat(paste0("... written: ", outFile, "\n"))
