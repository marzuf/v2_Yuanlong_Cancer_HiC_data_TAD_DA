...
plot_dt <- melt(tad_genepairs_dt[,c("ratioShared", "ratioOnlyHicds1", "ratioOnlyHicds2")])


plotTit <- paste0("Gene pairs from signif. TADs")

all_cmps <- unique(file.path(tad_genepairs_dt$hicds1, tad_genepairs_dt$hicds2, tad_genepairs_dt$exprds))
...
outFile <- file.path(outFolder, paste0("ratio_shared_gene_pairs_fromSignifTADs_density.", plotType))
...
outFile <- file.path(outFolder, paste0("ratio_shared_gene_pairs_fromSignifTADs_boxplot.", plotType))
ggsave(p3, file=outFile, height=myHeightGG, width=myWidthGG)
cat(paste0("... written: ", outFile, "\n"))
