...
# Rscript obs_tad_negative_fc.R

script_name <- "obs_tad_negative_fc.R"
...
outFolder <- file.path("OBS_TAD_NEGATIVE_FC")
...
if(buildData) {
  foo1 <- foreach(hicds = all_hicds) %do%{
    foo2 <- foreach(exprds = all_exprds[[paste0(hicds)]]) %do% {
      ...
      curr_tad = all_tads[1]
      all_obs_negFC <- foreach(curr_tad = all_tads, .combine='c') %dopar% {
      ...
      names(all_obs_negFC) <- all_tads
      outFile <- file.path(outFolder, hicds, exprds, "all_obs_negFC.Rdata")
      dir.create(dirname(outFile), recursive = TRUE)
      save(all_obs_negFC, file=outFile, version=2)
  ...
  outFile <- file.path(outFolder, hicds, exprds, "all_obs_negFC.Rdata")
  all_obs_negFC <- get(load(outFile))
}
