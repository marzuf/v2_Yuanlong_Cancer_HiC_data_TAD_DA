...
# Rscript go_signif_across_hicds_v2_shuffle.R
# Rscript go_signif_across_hicds_v2_shuffle.R norm_vs_tumor
# Rscript go_signif_across_hicds_v2_shuffle.R subtypes
# Rscript go_signif_across_hicds_v2_shuffle.R wt_vs_mut

script_name <- "go_signif_across_hicds_v2_shuffle.R"
...
outFolder <- file.path("GO_SIGNIF_ACROSS_HICDS_v2_SHUFFLE", data_cmpType)
...
inFolderObs <- "TAD_MATCHING_SIGNIF_ACROSS_HICDS_ALLMATCH_v2"
inFile <- file.path(inFolderObs, paste0(file_prefix, "signif_tads", signif_column, signifThresh, "_minBpRatio", minOverlapBpRatio, "_minInterGenes", minIntersectGenes, ".Rdata"))
cat(inFile, "\n")
stopifnot(file.exists(inFile))
obs_signif_tads <- get(load(inFile))

foo <- foreach(i_perm=1:nPerm) %dopar% {
  
  outFolder <- file.path("GO_SIGNIF_ACROSS_HICDS_v2_SHUFFLE", data_cmpType, i_perm)
  dir.create(outFolder, recursive = TRUE)
  
  inFolder <- file.path("TAD_MATCHING_SIGNIF_ACROSS_HICDS_ALLMATCH_v2_SHUFFLE", data_cmpType, i_perm)
  ...
  signif_tads <- unlist(lapply(1:length(dt), function(x) {
            all_regs <- unique(dt[[x]][["dataset_g2t_dt"]]$region)
            file.path(names(dt)[x], all_regs)}))  
            
  stopifnot(length(signif_tads) == length(obs_signif_tads))
