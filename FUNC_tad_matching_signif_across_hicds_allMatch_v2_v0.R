...
# Rscript FUNC_tad_matching_signif_across_hicds_allMatch_v2_v0.R  
...
overlapDT = overlapDT[order(as.character(overlapDT$ref_dataset), as.character(overlapDT$refID), as.character(overlapDT$query_dataset), as.character(overlapDT$queryID)),]
rownames(overlapDT) = NULL

outFile <- file.path(outFolder, paste0("overlapDT.Rdata"))
save(overlapDT, file=outFile, version=2)
