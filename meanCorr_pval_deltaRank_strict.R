...
registerDoMC(40)
require(ggpubr)
# Rscript meanCorr_pval_deltaRank_strict.R
...
all_ds_dt <- foreach(hicds = all_hicds, .combine='rbind') %dopar% {
  ...
  exprds_dt <- foreach(exprds = all_exprds[[paste0(hicds)]], .combine='rbind') %do% {
    ...
    tad_withRank_dt <- get(load(file.path(gsub("RANDOMMIDPOSSTRICT", "RANDOMMIDPOS", hicds), "ASSIGNED_REGIONS_DELTARANK", "all_assigned_regions_withDeltaRank.Rdata")))
