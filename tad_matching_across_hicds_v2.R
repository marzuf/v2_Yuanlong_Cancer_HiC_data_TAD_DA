...
# Rscript tad_matching_across_hicds_v2.R

script_name <- "tad_matching_across_hicds_v2.R"
...
for(overlapType in all_overlapTypes){
  ...
  # too slow... try with data.table
  
  # all_matching_onlyBest_dt <- do.call(rbind, lapply(split(all_matching_dt, 
  #                                                    list(all_matching_dt$ref_dataset, all_matching_dt$refID)),
  #                                       function(tmpDT){
  ...
  all_matching_onlyBest_dt <- foreach(curr_ds = all_levels, .combine='rbind') %do% {
    dataset_dt <- all_matching_dt[as.character(all_matching_dt$ref_dataset) == curr_ds,]
    all_tads <- unique(as.character(dataset_dt$refID))
    tad_dt <- foreach(ds_tad = all_tads, .combine='rbind') %dopar% {
      # as it is sorted, take only the 1st row
      sub_dt <- dataset_dt[dataset_dt$refID == ds_tad, ]
      sub_dt[1,,drop=FALSE]
    }
    tad_dt
  }
  ...
  # if(overlapType == "overlapBp") { # this version directly discards the NA
  #   
  #   all_matching_onlyBest_dt <- all_matching_dt[, .(matchingID_maxOverlapBp = matchingID_maxOverlapBp[which.max(overlapBp)],
  #                                                   overlapBp = overlapBp[which.max(overlapBp)],
  #                                                   overlapBp_ratio = overlapBp_ratio[which.max(overlapBp)]
  #   ), 
  #   by = list(ref_dataset, refID)]
  #   
  #   
  # } else if(overlapType == "nOverlapGenes") {
  #   
  #   all_matching_onlyBest_dt <- all_matching_dt[, .(matchingID_maxOverlapGenes = matchingID_maxOverlapGenes[which.max(nOverlapGenes)],
  #                                                   nOverlapGenes = nOverlapGenes[which.max(nOverlapGenes)],
  #                                                   nOverlapGenes_ratio = nOverlapGenes_ratio[which.max(nOverlapGenes)]
  #   ), 
  #   by = list(ref_dataset, refID)]
  #   
  #   
  ...
  all_matching_onlyBest_dt_v2 <- all_matching_onlyBest_dt
  outFile <- file.path(outFolder, "all_matching_onlyBest_dt_v2.Rdata") 
  save(all_matching_onlyBest_dt_v2, file = outFile)
  cat(paste0("... written: ", outFile))
