...
outFolder <- file.path(paste0("GIMAP_AND_PURITY", file_suffix))
...
if(buildTable){
  ...
  all_fpkm_gimap_purity_dt <- foreach(ds = all_ds, .combine='rbind') %dopar% {
    ...
    stopifnot(ncol(purity_expr_dt) == length(gimap_entrez) + 2)
    
    purity_expr_dt <- purity_expr_dt[,c("sampID", "purity", gimap_entrez)]
    purity_expr_dt
...
# load("GIMAP_AND_PURITY/all_fpkmGO_purity_dt.Rdata")
