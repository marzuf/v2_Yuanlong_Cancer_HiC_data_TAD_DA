...
# Rscript revision_rankDiff_activDiff_v3_notPF.R
...
script_name <- "revision_rankDiff_activDiff_v3_notPF.R"
...
outFolder <- file.path("REVISION_RANKDIFF_ACTIVDIFF_V3_NOTPF")
...
stopifnot(setequal(plot_dt$refID, cptmt_dt$refID))
...
for(threshtype in c("qtDist", "qtCptmt", "qtSameCptmt")) {
  ...
  plotTit <- paste0("TAD abs. rank diff. and signif. (not PF)")
  ...
  plotTit <- paste0("TAD abs. rank diff. and signif. with all (not PF)")
  
  p3 <- gghistogram(matching_withRank_dt_all,
                    x = paste0("abs_rankDiff"),
                    # y = "..density..",
                    y = "..density..",
                    bins =nHistBins,
