...
# Rscript ctcf_and_family_clusters_randomV3_filterLength.R
...
outFolder <- file.path("CTCF_AND_FAMILY_CLUSTERS_RANDOMV3_FILTERLENGTH", paste0("ctcf", ctcf_clustSize, "_family", family_clustSize))
...
qtFilterLength <- 0.9
qtFilterLengthValue <- quantile(final_fams_cluster_dt$clusterLength, probs=qtFilterLength)


outFile <- file.path(outFolder, "density_familiyLength_withFilter.svg")
svg(outFile, height=5,width=7)
plot(density(final_fams_cluster_dt$clusterLength), main="family length distribution")
mtext(side=3, text =paste0("length thresh.: ", round(qtFilterLength, 4), "-qt (=", round(qtFilterLengthValue, 4), "; #>: ",
                           sum(final_fams_cluster_dt$clusterLength > qtFilterLengthValue), ")"))
abline(v = qtFilterLengthValue, col="red")
foo <- dev.off()

final_fams_cluster_dt <- final_fams_cluster_dt[final_fams_cluster_dt$clusterLength <= qtFilterLengthValue ,]
