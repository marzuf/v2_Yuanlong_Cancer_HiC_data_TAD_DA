...
# Rscript revision_GO_byCptmt_withoutPurityFilter.R
...
resultData$region_id <- file.path(resultData$hicds, resultData$exprds, resultData$region)
...
outFolder <- file.path("REVISION_GO_BYCPTMT_WITHOUTPURITYFILTER")
...
all_data_list <- list(
  list(tad_list = resultData$region_id, data_dt = resultData, name="without PF")
)
# 
