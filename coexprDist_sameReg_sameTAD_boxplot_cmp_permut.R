startTime <- Sys.time()
cat(paste0("> Rscript coexprDist_sameReg_sameTAD_boxplot_cmp_permut.R\n"))

# Rscript coexprDist_sameReg_sameTAD_boxplot_cmp_permut.R
# Rscript coexprDist_sameReg_sameTAD_boxplot_cmp_permut.R ENCSR489OCU_NCI-H460_40kb TCGAluad_norm_luad
...
inFolder <- file.path("AUC_COEXPRDIST_WITHREG_SORTNODUP")
...
FOO <- foreach(hicds = all_hicds, .combine='rbind') %dopar% {
  ...
  FOO <- foreach(exprds = all_exprds[[paste0(hicds)]], .combine='rbind') %do% {
    ...
    outFolder <- file.path("COEXPRDIST_sameReg_SAMETAD_BOXPLOT_CMP_PERMUT", hicds, exprds)
    ...
    allData_dt <- get(load(file.path(inFolder, hicds, paste0(exprds), "allData_dt.Rdata")))
...
head(allData_dt)
    ...
    allData_dt$reg <- ifelse(allData_dt$reg == 1, "sameReg",
                                    ifelse(allData_dt$reg == 0, "diffReg", NA))
    stopifnot(!is.na(allData_dt$reg))
    
    m1 <- lm(coexpr ~ sameTAD, data = allData_dt)
    m2 <- lm(coexpr ~ reg, data = allData_dt)
    m3 <- lm(coexpr ~ sameTAD+reg, data = allData_dt)
    ...
    to_merge_dt <- allData_dt[allData_dt$reg == "sameReg" & allData_dt$sameTAD == "sameTAD",]
    ...
    for(rd_patt in rd_patterns) {
      ...
      allData_dt <- get(load(file.path(inFolder, rd_hicds, paste0(exprds), "allData_dt.Rdata")))
    ...
    stopifnot(merged_dt$reg == "sameReg")
    ...
    subTit <- paste0("sameReg+sameTAD gene pairs; max dist. = ", maxDist/1000, " kb")
    ...
    outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_cmp_obs_permut_sameReg_sameTAD_boxplot.", plotType))
    ...
    outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_cmp_obs_permut_sameReg_sameTAD_boxplot_jitter.", plotType))
    ggsave(plot = p_coexpr_boxplot_jitter, filename = outFile, height=myHeightGG, width = myWidthGG*1.5)
    cat(paste0("... written: ", outFile, "\n"))
    
    
    outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_cmp_obs_permut_sameReg_sameTAD_countPairs_dt.txt"))
    ...
    outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_cmp_obs_permut_sameReg_sameTAD_countPairs_barplot.", plotType))
