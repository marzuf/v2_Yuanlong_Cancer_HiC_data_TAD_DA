...
inFolder <- file.path("TAD_MATCHING_SIGNIF_ACROSS_HICDS_ALLMATCH_v2_RANDOMMIDPOSSTRICT", data_cmpType)
...
if(length(go_all_conserved_signif_tads_genes) > 0) {
  ...
  if(genes_signif_plotMax > 0) {
    for(var_plot in barplot_vars) {
      myTit <- paste0(barplot_vars_tit[var_plot], " ", data_cmpType, " (conserved_signif) - RANDOMMIDPOSSTRICT")
...
if(length(go_all_not_conserved_signif_tads_genes) > 0) {
  ...
  if(genes_signif_plotMax > 0) {
    for(var_plot in barplot_vars) {
      myTit <- paste0(barplot_vars_tit[var_plot], " ", data_cmpType, " (not_conserved_signif) - RANDOMMIDPOSSTRICT")
