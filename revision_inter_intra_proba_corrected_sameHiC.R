...
# Rscript revision_inter_intra_proba_corrected_sameHiC.R
script_name="revision_inter_intra_proba_corrected_sameHiC.R"
startTime <- Sys.time()
...
outFolder <- "REVISION_INTER_INTRA_PROBA_CORRECTED_SAMEHIC"
...
if(buildTable) {
  all_inter_intra_dt <- foreach(i_ds = seq_along(all_pairs), .combine='rbind') %do% {
    ...
    all_chromo_dt <- foreach(chromo = all_chrs,.combine='rbind') %dopar% {
      ...
      cat(paste0("... performing scale normalization...\n"))
      ...
      # stopifnot(!is.na(matNorm_dt$normCount)) # not true v1 if 1 value in scale na
      ...
      # stopifnot(!is.na(matched_matNorm_dt$normCount)) # not true v1 if 1 value scale returns na
