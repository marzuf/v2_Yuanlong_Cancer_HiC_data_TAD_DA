...
outFolder <- file.path("CTCF_AND_DA_CURVES_V2")
...
for(i in 1:length(all_to_plot)) {
  ...
  agg_all_dt <- aggregate(as.formula(paste0(curr_col, " ~ rel_pos_lab + signif_lab")), data=agg_byTAD_dt, FUN = "sum")
  ...
  agg_all_dt <- aggregate(as.formula(paste0(curr_col, " ~ rel_pos_lab + Triplet_class")), data=agg_byTAD_dt, FUN = "sum")
  ...
  agg_all_dt <- aggregate(as.formula(paste0(curr_col, " ~ rel_pos_lab + orientation")), data=agg_byTAD_dt, FUN = "sum")
