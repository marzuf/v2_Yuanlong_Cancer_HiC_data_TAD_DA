...
script_name <- "obs_permut_familyTADs_allDS_min3FamsTADs.R"

# Rscript obs_permut_familyTADs_allDS_min3FamsTADs.R
...
outFolder <- "OBS_PERMUT_FAMILY_TADS_ALLDS_MIN3FAMSTADS"
...
if(buildData) {
  all_data <- foreach(hicds = all_hicds) %dopar% {
    ...
    exprds_data <- foreach(exprds = all_exprds[[paste0(hicds)]]) %do% {
      ...
      # save(fam_pip_dt, file="fam_pip_dt.Rdata", version=2)
      # UPDATE HERE TO KEEP ONLY TADs WITH AT LEAST 3 FAM ANNOTATED
      
      nAnnotByTADs <- setNames(as.numeric(table(fam_pip_dt$region)), names(table(fam_pip_dt$region)))
      keepTADs <- names(nAnnotByTADs)[nAnnotByTADs >= minByTAD ]
      
      fam_pip_dt <- fam_pip_dt[fam_pip_dt$region %in% keepTADs,]
