...
# Rscript tfsets_and_tads_allDS_onlyDE.R crisp 
# Rscript tfsets_and_tads_allDS_onlyDE.R c3.mir
# Rscript tfsets_and_tads_allDS_onlyDE.R c3.mir LG1_40kb
# Rscript tfsets_and_tads_allDS_onlyDE.R c3.tft
# Rscript tfsets_and_tads_allDS_onlyDE.R c3.all
# Rscript tfsets_and_tads_allDS_onlyDE.R trrust
# Rscript tfsets_and_tads_allDS_onlyDE.R tftg
# Rscript tfsets_and_tads_allDS_onlyDE.R motifmap LG1_40kb
# Rscript tfsets_and_tads_allDS_onlyDE.R motifmap 
# Rscript tfsets_and_tads_allDS_onlyDE.R kegg LG1_40kb
...
if(length(args) == 3) {
  all_hicds <- args[2]
  all_exprds <- setNames(args[3], all_hicds)
...
outFolder <- file.path(paste0("TFSETS_AND_TADS_ONYLDE", toupper(dsIn)))
...
for(hicds in all_hicds){
  ...
  for(exprds in all_exprds[paste0(hicds)]) {
    ...
    if(buildData) {
      ...
      topDT <- get(load(file.path("PIPELINE", "OUTPUT_FOLDER", hicds, exprds, "1_runGeneDE", "DE_topTable.Rdata")))
      
      stopifnot(names(geneList) %in% topDT$genes  )
      
      de_genes <- topDT$genes[topDT$adj.P.Val <= geneSignifThresh]
      de_entrez <- geneList[names(geneList) %in% de_genes]
      ...
      stopifnot(de_entrez %in% gff_dt$entrezID)
      gff_dt <- gff_dt[gff_dt$entrezID %in% de_entrez,]
      ...
      outFile <- file.path(outFolder, hicds, exprds, "permut_dt.Rdata")
      dir.create(dirname(outFile), recursive = TRUE)
      save(permut_dt, file = outFile)
      cat(paste0("... written: ", outFile, "\n"))
      
      outFile <- file.path(outFolder, hicds, exprds, "obs_dt.Rdata")
      dir.create(dirname(outFile), recursive = TRUE)
      save(obs_dt, file = outFile)
      cat(paste0("... written: ", outFile, "\n"))
      
    } else {
      inFile <- file.path(outFolder, hicds, exprds, "permut_dt.Rdata")
      permut_dt <- get(load(inFile))
      inFile <- file.path(outFolder, hicds, exprds, "obs_dt.Rdata")
    ...
    outFile <- file.path(outFolder, hicds, exprds, paste0("obs_permut_boxplot.", plotType))
    ...
    outFile <- file.path(outFolder, hicds, exprds, paste0("obs_permut_multidens.", plotType))
    do.call(plotType, list(outFile, height=myHeight, width=myWidth))
    ...
    outFile <- file.path(outFolder, hicds, exprds, paste0("obs_permut_boxplot_to100.", plotType))
    do.call(plotType, list(outFile, height=myHeight, width=myWidth))
    ...
    outFile <- file.path(outFolder, hicds, exprds, paste0("obs_permut_multidens_to100.", plotType))
    do.call(plotType, list(outFile, height=myHeight, width=myWidth))
