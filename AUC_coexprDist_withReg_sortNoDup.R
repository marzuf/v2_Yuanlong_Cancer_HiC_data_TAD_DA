startTime <- Sys.time()
cat(paste0("> Rscript AUC_coexprDist_withReg_sortNoDup.R\n"))
...
outFold <- file.path("AUC_COEXPRDIST_WITHREG_SORTNODUP",  curr_TADlist, paste0(curr_dataset))
dir.create(outFold, recursive = TRUE)

logFile <- file.path(outFold, paste0("coexpr_dist_withReg_logFile.txt"))  
file.remove(logFile)
...
mycols <- c("same TAD" ="darkorange1" , "diff. TAD"="darkslateblue",  "same Reg + same TAD"="violetred1", "same Reg + diff. TAD" = "lightskyblue")

sameTADcol <- mycols["same TAD"]
diffTADcol <- mycols["diff. TAD"]
sameRegSameTADcol <- mycols["same Reg + same TAD"]
sameRegDiffTADcol <- mycols["same Reg + diff. TAD"]
...
regFile <- file.path("CREATE_REG_SORTNODUP", "all_reg_pairs.Rdata")
stopifnot(file.exists(regFile))
...
printAndLog(txt, logFile)
txt <- paste0("... sameReglogFile = ",  logFile, "\n")
printAndLog(txt, logFile)
...
if(buildTable) {
  ...
  if(buildTable){
    ...
    cat(paste0(Sys.time(), "\n"))
    dataset_dist_TAD_tf_DT$reg <- ifelse(is.na(dataset_dist_TAD_tf_DT$reg), 0, 1)
  ...
  allData_dt$curve2 <-  ifelse(allData_dt$reg == "0", NA,
                             ifelse(allData_dt$sameTAD == "0", "same Reg + diff. TAD", "same Reg + same TAD"))
  ...
  sameReg_sameTAD_DT <- allData_dt[allData_dt$reg == 1 & allData_dt$sameTAD == 1 ,c("gene1", "gene2", "coexpr", "dist",  "dist_kb")]
  sameReg_sameTAD_DT <- na.omit(sameReg_sameTAD_DT)
  sameReg_sameTAD_DT <- sameReg_sameTAD_DT[order(sameReg_sameTAD_DT$dist_kb),]
  sameReg_sameTAD_DT$nPair <- 1:nrow(sameReg_sameTAD_DT)
  sameReg_sameTAD_DT$label <- "same Reg + same TAD"
  
  sameReg_diffTAD_DT <- allData_dt[allData_dt$reg == 1 & allData_dt$sameTAD == 0 ,c("gene1", "gene2",  "coexpr", "dist", "dist_kb")]
  sameReg_diffTAD_DT <- na.omit(sameReg_diffTAD_DT)
  sameReg_diffTAD_DT <- sameReg_diffTAD_DT[order(sameReg_diffTAD_DT$dist_kb),]
  sameReg_diffTAD_DT$nPair <- 1:nrow(sameReg_diffTAD_DT)
  sameReg_diffTAD_DT$label <- "same Reg + diff. TAD"
  ...
  stopifnot(is.numeric(sameReg_sameTAD_DT$dist[1]))
  stopifnot(is.numeric(sameReg_sameTAD_DT$coexpr[1]))
  stopifnot(is.numeric(sameReg_diffTAD_DT$dist[1]))
  stopifnot(is.numeric(sameReg_diffTAD_DT$coexpr[1]))
  ...
  if(fitMeth == "loess") {
    ...
    mtext(text = paste0("observed distance values"), side = 3)
    ...
    ################################ DO THE SAME FOR same Reg SAME TAD VS. same Reg DIFF TAD
    
    
    sameRegDiffTAD_mod <- loess(coexpr ~ dist, data = sameReg_diffTAD_DT)
    sameRegSameTAD_mod <- loess(coexpr ~ dist, data = sameReg_sameTAD_DT)
    
    
    smooth_vals_sameRegSameTAD <- predict(sameRegSameTAD_mod, sort(sameReg_sameTAD_DT$dist))
    smooth_vals_sameRegDiffTAD <- predict(sameRegDiffTAD_mod, sort(sameReg_diffTAD_DT$dist))
    
    auc_sameRegDiffTAD_obsDist <- auc(x = sort(sameReg_diffTAD_DT$dist), y = smooth_vals_sameRegDiffTAD)
    auc_sameRegSameTAD_obsDist <- auc(x = sort(sameReg_sameTAD_DT$dist), y = smooth_vals_sameRegSameTAD)
    
    outFile <- file.path(outFold, paste0(curr_TADlist, "_", curr_dataset, "_sameRegSameTAD_tfDiffTAD_loessFit_originalDist", ".", plotType))
    do.call(plotType, list(outFile, height = myHeight, width = myWidth))
    plot(NULL,
         xlim = range(allData_dt$dist), 
         ylim = range(c(smooth_vals_sameRegSameTAD, smooth_vals_sameRegDiffTAD)),
         xlab=my_xlab, 
         ylab=my_ylab,
         main=paste0(curr_TADlist, " - ", curr_dataset, ": coexpr ~ dist loess fit"))
    mtext(text = paste0("observed distance values"), side = 3)
    lines( x = sort(sameReg_sameTAD_DT$dist), y = smooth_vals_sameRegSameTAD, col = sameRegSameTADcol)
    lines( x = sort(sameReg_diffTAD_DT$dist), y = smooth_vals_sameRegDiffTAD, col = sameRegDiffTADcol)
    legend("topright", 
           legend=c(paste0("sameRegSameTAD\n(AUC=", round(auc_sameRegSameTAD_obsDist, 2), ")"), 
                    paste0("sameRegDiffTAD\n(AUC=", round(auc_sameRegDiffTAD_obsDist, 2))),  
           col = c(sameRegSameTADcol, sameRegDiffTADcol),
           lty=1,
           bty = "n")
    ...
    # PREDICT WITH DISTANCE VECTOR
    smooth_vals_sameRegSameTAD_distVect <- predict(sameRegSameTAD_mod, distVect)
    smooth_vals_sameRegDiffTAD_distVect <- predict(sameRegDiffTAD_mod, distVect)
    
    auc_sameRegDiffTAD_distVect <- auc(x = distVect, y = smooth_vals_sameRegDiffTAD_distVect)
    auc_sameRegSameTAD_distVect <- auc(x = distVect, y = smooth_vals_sameRegSameTAD_distVect)
    ...
    outFile <- file.path(outFold, "sameRegDiffTAD_mod.Rdata")
    save(sameRegDiffTAD_mod, file = outFile)
    cat(paste0("... written: ", outFile, "\n"))
    ...
    outFile <- file.path(outFold, "sameRegSameTAD_mod.Rdata")
    save(sameRegSameTAD_mod, file = outFile)
    cat(paste0("... written: ", outFile, "\n"))
    
    
    sameRegDiffTAD_obsDist <- sameReg_diffTAD_DT$dist
    outFile <- file.path(outFold, "sameRegDiffTAD_obsDist.Rdata")
    save(sameRegDiffTAD_obsDist, file = outFile)
    cat(paste0("... written: ", outFile, "\n"))
    
    
    sameRegSameTAD_obsDist <- sameReg_sameTAD_DT$dist
    outFile <- file.path(outFold, "sameRegSameTAD_obsDist.Rdata")
    save(sameRegSameTAD_obsDist, file = outFile)
    cat(paste0("... written: ", outFile, "\n"))
    
    
    outFile <- file.path(outFold, "auc_sameRegDiffTAD_distVect.Rdata")
    save(auc_sameRegDiffTAD_distVect, file = outFile)
    cat(paste0("... written: ", outFile, "\n"))
    
    outFile <- file.path(outFold, "auc_sameRegSameTAD_distVect.Rdata")
    save(auc_sameRegSameTAD_distVect, file = outFile)
    cat(paste0("... written: ", outFile, "\n"))
    
    outFile <- file.path(outFold, "smooth_vals_sameRegSameTAD_distVect.Rdata")
    save(smooth_vals_sameRegSameTAD_distVect, file = outFile)
    cat(paste0("... written: ", outFile, "\n"))
    
    outFile <- file.path(outFold, "smooth_vals_sameRegDiffTAD_distVect.Rdata")
    save(smooth_vals_sameRegDiffTAD_distVect, file = outFile)
    cat(paste0("... written: ", outFile, "\n"))
    
    outFile <- file.path(outFold, paste0(curr_TADlist, "_", curr_dataset, "_sameRegSameTAD_sameRegDiffTAD_loessFit_vectDist.", plotType))
    do.call(plotType, list(outFile, height = myHeight, width = myWidth))
    plot(NULL,
         xlim = range(distVect), 
         ylim = range(c(na.omit(smooth_vals_sameRegSameTAD_distVect), na.omit(smooth_vals_sameRegDiffTAD_distVect))),
         # xlab="", 
         # ylab="",
         xlab=my_xlab,
         ylab=my_ylab,
         main=paste0(curr_TADlist, " - ", curr_dataset, ": coexpr ~ dist loess fit"))
    mtext(text = paste0("distance values seq from 0 to ", distLimit, " (# points = ", nbrLoessPoints, ")"), side = 3)
    lines( x = distVect, y = smooth_vals_sameRegSameTAD_distVect, col = sameRegSameTADcol)
    lines( x = distVect, y = smooth_vals_sameRegDiffTAD_distVect, col = sameRegDiffTADcol)
    legend("topright", 
           legend=c(paste0("sameRegSameTAD\n(AUC=", round(auc_sameRegSameTAD_distVect, 2), ")"), 
                    paste0("sameRegDiffTAD\n(AUC=", round(auc_sameRegDiffTAD_distVect, 2))), 
           col = c(sameRegSameTADcol, sameRegDiffTADcol),
           lty=1,
           bty = "n")
    ...
    ################################################################ 
    outFile <- file.path(outFold, paste0(curr_TADlist, "_", curr_dataset, "_sameTAD_diffTAD_sameRegSameTAD_sameRegDiffTAD_loessFit_vectDist.", plotType))
    do.call(plotType, list(outFile, height = myHeight, width = myWidth))
    plot(NULL,
         xlim = range(distVect), 
         ylim = range(c(na.omit(smooth_vals_sameTAD_distVect), 
                        na.omit(smooth_vals_sameRegSameTAD_distVect),
                        na.omit(smooth_vals_diffTAD_distVect),
                        na.omit(smooth_vals_sameRegDiffTAD_distVect))),
         # xlab="", 
         # ylab="",
         xlab=my_xlab,
         ylab=my_ylab,
    ...
    legend("topright", 
           legend=c(paste0("sameTAD\n(AUC=", round(auc_sameTAD_distVect, 2), ")"), 
                    paste0("diffTAD\n(AUC=", round(auc_diffTAD_distVect, 2)), 
                     paste0("sameRegSameTAD\n(AUC=", round(auc_sameRegSameTAD_distVect, 2), ")"), 
                     paste0("sameRegDiffTAD\n(AUC=", round(auc_sameRegDiffTAD_distVect, 2))), 
           col = c(sameTADcol, diffTADcol,
                   sameRegSameTADcol, sameRegDiffTADcol),
           lty=1,
           bty = "n")
    ...
    auc_values <- list(
      ...
      auc_sameRegDiffTAD_distVect = auc_sameRegDiffTAD_distVect,
      auc_sameRegSameTAD_distVect = auc_sameRegSameTAD_distVect,
      auc_ratio_sameReg_same_over_diff_distVect = auc_sameRegSameTAD_distVect/auc_sameRegDiffTAD_distVect,
      auc_sameRegDiffTAD_obsDist = auc_sameRegDiffTAD_obsDist,
      auc_sameRegSameTAD_obsDist = auc_sameRegSameTAD_obsDist,
      auc_ratio_sameReg_same_over_diff_obsDist = auc_sameRegSameTAD_distVect/auc_sameRegDiffTAD_obsDist
    )
