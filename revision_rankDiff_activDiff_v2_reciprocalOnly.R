...
# Rscript revision_rankDiff_activDiff_v2_reciprocalOnly.R

## reciprocalonly version: keep only reciprocal best matching pairs
...
script_name <- "revision_rankDiff_activDiff_v2_reciprocalOnly.R"
...
outFolder <- file.path("REVISION_RANKDIFF_ACTIVDIFF_V2_RECIPROCALONLY")
...
########################################
### ADD FILTER HERE TO KEEP ONLY RECIPROCAL BEST MATCH


ds_pairs <- apply(matching_withRank_dt, 1, function(x) {
  paste0(sort(c(as.character(x["ref_region_ID"]), as.character(x["matching_region_ID"]))), collapse="_")
})
stopifnot(length(ds_pairs) == nrow(matching_withRank_dt))
mean(table(ds_pairs) == 1) # 0.2198325
mean(table(ds_pairs) == 2) # 0.7801675
stopifnot(table(ds_pairs) == 1 | table(ds_pairs) == 2)

pairs_to_keep <- names(table(ds_pairs))[table(ds_pairs) == 2]

matching_withRank_dt$pairID <- ds_pairs

filtered_matching_withRank_dt <- matching_withRank_dt[matching_withRank_dt$pairID %in% pairs_to_keep,]
stopifnot(nrow(filtered_matching_withRank_dt) > 0)

nrow(filtered_matching_withRank_dt)/nrow(matching_withRank_dt)
# 0.8765102

matching_withRank_dt <- filtered_matching_withRank_dt

stopifnot(table(matching_withRank_dt$pairID) == 2)
