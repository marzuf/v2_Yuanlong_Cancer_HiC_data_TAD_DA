...
# Rscript notSignif_conserved_regions_and_synteny.R
# Rscript notSignif_conserved_regions_and_synteny.R norm_vs_tumor
# Rscript notSignif_conserved_regions_and_synteny.R subtypes
# Rscript notSignif_conserved_regions_and_synteny.R wt_vs_mut

# Rscript notSignif_conserved_regions_and_synteny.R <cmpType>

cat("> START ", "notSignif_conserved_regions_and_synteny.R", "\n")
...
outFolder <- file.path("NOTSIGNIF_CONSERVED_REGIONS_AND_SYNTENY", cmpType)
...
inFile <- file.path("CREATE_COORD_CONSERVED_NOTSIGNIF_REGIONS", cmpType, paste0(filePrefix, "conserved_notSignif_tads_coord.Rdata"))
...
inFile <- file.path("TAD_MATCHING_NOTSIGNIF_ACROSS_HICDS_ALLMATCH_v2", cmpType, paste0("plot_matching_dt.Rdata"))
cat(inFile,"\n")
stopifnot(file.exists(inFile))
...
for(synType in c("inferCARs", "proCARs")) {
  ...
  plot(nConserv_nSyn_dt$nConserv ~ nConserv_nSyn_dt$nUniqueSyn,
       ...
       main = paste0("not signif. conserv. and synteny - ", cmpTit),
       pch=16,
       cex=0.7)
  ...
  # nConserv_nSyn_dt$hasSynteny <- as.character(nConserv_nSyn_dt$nSyn > 0)
  # tmp_dt <- aggregate(refID ~ nConserv + hasSynteny, data = nConserv_nSyn_dt, FUN=length)
  # boxplot(refID~hasSynteny+nConserv, data = tmp_dt, las=2)
