########################################################################################################################################################################################
startTime <- Sys.time()
cat(paste0("> Rscript allTADs_and_purity_final_permutg2t.R\n"))

script_name <- "allTADs_and_purity_final_permutg2t.R"
...
all_result_dt <- get(load("CREATE_FINAL_TABLE_PERMUT//all_result_dt.Rdata"))
...
outFolder <- file.path("ALLTADS_AND_PURITY_FINAL_PERMUT", purity_ds, pm, transfExpr)
...
all_hicds <- all_hicds[grepl("PERMUTG2T", all_hicds)]
