...
# Rscript revision_aggChipPeaks_data_fc2.R

outFolder <- "REVISION_AGGCHIPPEAKS_DATA_FC2"
...
hicds_chips <- c( "GSE118514_RWPE1_40kb" = "RWPE1_ENCFF039XYU",
                "GSE118514_22Rv1_40kb" = "22Rv1_ENCFF286BKT")
# 22Rv1_ENCFF286BKT_fcOverControl.bedGraph
# RWPE1_ENCFF039XYU_fcOverControl.bedGraph

peakFolder <- "prostate_chip_seq"
ref_peaks_file <- file.path(peakFolder, paste0(hicds_chips[paste0(ref_hicds)], "_fcOverControl.bedGraph"))
ref_peaks_dt <- read.delim(ref_peaks_file, header=FALSE, col.names = 
                           ...
                           c("chrom" ,"chromStart", "chromEnd" ,"fcOverControl"),
...
match_peaks_file <- file.path(peakFolder, paste0(hicds_chips[paste0(match_hicds)], "_fcOverControl.bedGraph"))
...
ref_final_dt$match_meanFC <- ref_final_dt$ref_meanFC <- NA
...
tad_withPeaks_dt <- foreach(i_tad = 1:nrow(ref_final_dt), .combine='rbind') %dopar% {
  ...
  # if(nrow(sub_match_dt) > 0 ) {
    ...
    ref_final_dt$match_meanFC[i_tad] <- mean(sub_match_dt$fcOverControl)
  ...
  # if(nrow(sub_match_dt) > 0 ) {
    ...
    ref_final_dt$ref_meanFC[i_tad] <- mean(sub_ref_dt$fcOverControl)
