...
# Rscript nbr_signif_all_sameNbr.R

outFolder <- "NBR_SIGNIF_ALL_SAMENBR"
...
all_dt <- foreach(hicds = all_hicds, .combine='rbind') %dopar% {
  ...
  exprds_dt <- foreach(exprds = all_exprds[[paste0(hicds)]], .combine='rbind') %do% {
    ...
    combPval_file <- file.path(pipOutFolder, hicds, exprds, "11sameNbrSameNbr_runEmpPvalCombined", "emp_pval_combined.Rdata")
