########################################################################################################################################################################################
startTime <- Sys.time()
cat(paste0("> Rscript purity_available_samples_final.R\n"))

script_name <- "purity_available_samples_final.R"
...
### HARD-CODED - MAIN SETTINGS - discussion 04.08.2020 Giovanni - keep aran CPE data, first vial only

# Rscript purity_available_samples_final.R 
...
outFolder <- file.path("PURITY_AVAILABLE_SAMPLES_FINAL", purity_ds, pm)
...
if(buildTable){
  ...
  all_ds_avPurity_dt <- foreach(ds = all_ds, .combine='rbind') %dopar% {
    ...
    av_samp2 <- samp2[ samp2 %in% purity_dt$Sample.ID]
    match_samp2 <- sapply(av_samp2, function(x) {
      sampmatch <- purity_dt$Sample.ID[purity_dt$Sample.ID == x]
      if(length(sampmatch) == 0) {
        sampmatch <- purity_dt$Sample.ID[purity_dt$Sample.ID == paste0(x)]
      }
      stopifnot(length(sampmatch) == 1)
      return(sampmatch)
    })
    missing_samp2 <- samp2[! (samp2 %in% purity_dt$Sample.ID)]
    
    
    av_samp1 <- samp1[samp1 %in% purity_dt$Sample.ID]
    ...
    missing_samp1 <- samp1[! ( samp1 %in% purity_dt$Sample.ID)]
