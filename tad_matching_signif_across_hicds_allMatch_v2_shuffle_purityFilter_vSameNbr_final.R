...
# Rscript tad_matching_signif_across_hicds_allMatch_v2_shuffle_purityFilter_vSameNbr_final.R
# Rscript tad_matching_signif_across_hicds_allMatch_v2_shuffle_purityFilter_vSameNbr_final.R norm_vs_tumor
# Rscript tad_matching_signif_across_hicds_allMatch_v2_shuffle_purityFilter_vSameNbr_final.R subtypes
# Rscript tad_matching_signif_across_hicds_allMatch_v2_shuffle_purityFilter_vSameNbr_final.R wt_vs_mut
...
script_name <- "tad_matching_signif_across_hicds_allMatch_v2_shuffle_purityFilter_vSameNbr_final.R"
...
outFolder <- file.path("TAD_MATCHING_SIGNIF_ACROSS_HICDS_ALLMATCH_v2_SHUFFLE_PURITYFILTER_VSAMENBR_FINAL", data_cmpType, purity_ds, pm, transfExpr)
...
save(TADs_to_discard, file=file.path(outFolder, "TADs_to_discard.Rdata"))
...
####################################################################################################################################### >>> prepare the data
  ...
  foo_permut <- foreach(i_perm=1:nPermut) %dopar% {
    ...
    if(file.exists(outFile)) cat(paste0("skipped ", i_perm, "\n"))
    ...
    if(file.exists(outFile)) return(NULL)
        ...
        ### => update here, discard "pseudo" signif tads that are in signifTADs_discard # v0 -> sample as many signif
        ##### MAIN UPDATE HERE -> TO REMOVE THE TADS - 14.07.2020 ######################################## 
        # in this version -> remove the signif_tads and sample the same number
        
        ds_TADs_to_discard <- basename(TADs_to_discard)[dirname(TADs_to_discard) == file.path(hicds, exprds)]
        
        ftd_not_signif_tads <- not_signif_tads[ ! not_signif_tads %in% ds_TADs_to_discard]
        
        ftd_signif_tads <- signif_tads[ ! signif_tads %in% ds_TADs_to_discard]
        
        # sample as many filtered signif_tads in the already filtered not signif tads
        ftd_pseudosignif_tads <- sample(ftd_not_signif_tads, size=length(ftd_signif_tads))
        
        nSignifTADs <- length(signif_tads)
        nFtdSignifTADs <- length(ftd_signif_tads)
        
        stopifnot(length(ftd_pseudosignif_tads) == length(ftd_signif_tads))
        stopifnot(!ftd_pseudosignif_tads %in% signif_tads)
        stopifnot(!ftd_pseudosignif_tads %in% ftd_signif_tads)
        stopifnot(!ftd_pseudosignif_tads %in% ds_TADs_to_discard)

        signif_tads <- ftd_pseudosignif_tads
