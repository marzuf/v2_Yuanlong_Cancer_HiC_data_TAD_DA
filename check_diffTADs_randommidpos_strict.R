...
# Rscript check_diffTADs_randommidpos_strict.R
...
script_name <- "check_diffTADs_randommidpos_strict.R"
...
# with 0.5 threshold -> around ~200 keepTADs
# with 0.7 threshold -> around ~900 keepTADs
# with 0.8 -> only 3 datasets below 1000 keepTADs
  
outFolder <- file.path("CHECK_DIFFTADS_RANDOMMIDPOS_STRICT", ratioSameTAD_thresh)
...
all_result_dt <- foreach(hicds = all_hicds, .combine='rbind') %dopar% {
  ...
  hicds_dt <- foreach(exprds = all_exprds[[paste0(hicds)]], .combine='rbind') %do% {
    ...
    maxRatioRegByReg_dt <- aggregate(init_region ~ region, data =cmp_init_g2t_dt, FUN = function(x) {
    ...
    stopifnot(length(keepTADs) == sum(maxRatioRegByReg_dt$init_region <= ratioSameTAD_thresh))
