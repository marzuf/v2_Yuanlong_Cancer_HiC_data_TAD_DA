...
#### density with 3 cols
# Rscript revision_expressionLevel_cptmts_notPF_v4.R

outFolder <- file.path("REVISION_EXPRESSIONLEVEL_CPTMTS_NOTPF_V4")
...
cptmt_var = all_cptmt_vars[1]
plot_var = all_plot_vars[1]
...
for(cptmt_var in all_cptmt_vars){
  ...
  for(plot_var in all_plot_vars) {
    ...
    ############################################################### ADD SIGNIF WITH COLOR
    # mySub2 <- tad2cptmt_dt$adjPvalComb <= tadSignifThresh 
    tad2cptmt_dt$tadsignif <- tad2cptmt_dt$adjPvalComb <= tadSignifThresh
    tad2cptmt_dt$tadsignif_lab <- ifelse(tad2cptmt_dt$tadsignif, "signif.", "not signif.")
    
    ctgttab<- table(tad2cptmt_dt[,paste0(cptmt_var)],tad2cptmt_dt$tadsignif)
    print(paste0(plotTit, "\n"))
       print(ctgttab)
    ...
    #### add the signif here
    pbox <- pbox+ geom_jitter(data=tad2cptmt_dt[tad2cptmt_dt$adjPvalComb <= tadSignifThresh,], 
                              width=.25,
                              aes_string(x=paste0(cptmt_var),
                                         y=paste0(plot_var)),
                              color=signifDotCol)
    
    
    outFile <- file.path(outFolder,paste0(plot_var, "_byCptmt_", cptmt_var, "_withSignif_boxplot.", plotType))
    ...
    outFile <- file.path(outFolder,paste0(plot_var, "_byCptmt_", cptmt_var, "_withSignif_violinplot.", plotType))
    ggsave(pbox, filename = outFile, height=myHeightGG, width=myWidthGG)
    cat(paste0("... written: ", outFile, "\n"))
    ...
    pbox <- ggboxplot(tad2cptmt_dt, 
      ...
      # guides(color=FALSE)+
      scale_y_continuous(breaks = scales::pretty_breaks(n = 10))+
      facet_wrap(cptmt_var)
    

    outFile <- file.path(outFolder,paste0(plot_var, "_byCptmt_", cptmt_var, "_signifWrap_boxplot.", plotType))
    ggsave(pbox, filename = outFile, height=myHeightGG, width=myWidthGG*1.5)
    cat(paste0("... written: ", outFile, "\n"))
    ...
    outFile <- file.path(outFolder,paste0(plot_var, "_byCptmt_", cptmt_var, "_signifWrap_violinplot.", plotType))
    ggsave(pbox, filename = outFile, height=myHeightGG, width=myWidthGG*1.5)
    cat(paste0("... written: ", outFile, "\n"))
    ...
    tad2cptmt_dt_A <- tad2cptmt_dt[grepl("^A", tad2cptmt_dt[,cptmt_var]),]
    mySub <- paste0("# DS = ", length(unique(file.path(tad2cptmt_dt_A$hicds,tad2cptmt_dt_A$exprds))),
                    "; # TADs = ", nrow(tad2cptmt_dt_A))
    
    
    pbox <- ggviolin(tad2cptmt_dt_A, 
                     add = "boxplot",
                     # x=paste0(cptmt_var),
                     x=paste0("tadsignif_lab"),
                     y=paste0(plot_var),
    ...
    outFile <- file.path(outFolder,paste0(plot_var, "_byCptmt_", cptmt_var, "_A_signifWrap_violinplot.", plotType))
    ggsave(pbox, filename = outFile, height=myHeightGG, width=myWidthGG*1.5)
    cat(paste0("... written: ", outFile, "\n"))
    ...
    tad2cptmt_dt_B <- tad2cptmt_dt[grepl("^B", tad2cptmt_dt[,cptmt_var]),]
    mySub <- paste0("# DS = ", length(unique(file.path(tad2cptmt_dt_B$hicds,tad2cptmt_dt_B$exprds))),
                    "; # TADs = ", nrow(tad2cptmt_dt_B))
    
    
    pbox <- ggviolin(tad2cptmt_dt_B, 
                     add = "boxplot",
                     # x=paste0(cptmt_var),
                     x=paste0("tadsignif_lab"),
                     y=paste0(plot_var),
    ...
    outFile <- file.path(outFolder,paste0(plot_var, "_byCptmt_", cptmt_var, "_B_signifWrap_violinplot.", plotType))
    ggsave(pbox, filename = outFile, height=myHeightGG, width=myWidthGG*1.5)
    cat(paste0("... written: ", outFile, "\n"))
