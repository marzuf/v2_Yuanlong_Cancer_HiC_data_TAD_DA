startTime <- Sys.time()
cat(paste0("> Rscript AUC_coexprDist_Reg_boxplot.R\n"))

# Rscript AUC_coexprDist_reg_boxplot.R ENCSR079VIJ_G401_40kb TCGAkich_norm_kich
...
outFolder <- file.path("AUC_COEXPRDIST_WITHREG_BOXPLOT", hicds, exprds)
...
allData_dt$reg <- ifelse(allData_dt$reg == 1, "shared Reg",
                             ifelse(allData_dt$reg == 0, "no shared Reg", NA))
...
allData_dt$cond <- interaction(allData_dt$sameTAD, allData_dt$reg)

allData_dt$cond <- factor(allData_dt$cond, levels = c( "sameTAD.shared Reg", "sameTAD.no shared Reg",   "diffTAD.shared Reg", "diffTAD.no shared Reg"))
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_TAD_Reg_bboxplot.", plotType))
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_TAD_Reg_boxplot_jitter.", plotType))
ggsave(plot = p_coexpr_boxplot_jitter, filename = outFile, height=myHeightGG, width = myWidthGG*1.5)
cat(paste0("... written: ", outFile, "\n"))


outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_TAD_Reg_countPairs_dt.txt"))
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_TAD_Reg_countPairs_barplot.", plotType))
