startTime <- Sys.time()
cat(paste0("> Rscript AUC_coexprDist_exprTF_boxplot_permut.R\n"))

# Rscript AUC_coexprDist_exprTF_boxplot_permut.R chea3_lung ENCSR489OCU_NCI-H460_40kb TCGAluad_norm_luad
...
outFolder <- file.path("AUC_COEXPRDIST_WITHEXPRTF_BOXPLOT_PERMUT", dsIn, hicds, exprds)
dir.create(outFolder, recursive = TRUE)

inFolder <- file.path("AUC_COEXPRDIST_WITHEXPRTF_SORTNODUP_PERMUT", dsIn)
...
allData_dt$tf <- ifelse(allData_dt$tf == 1, "shared TF",
                             ifelse(allData_dt$tf == 0, "no shared TF", NA))
stopifnot(!is.na(allData_dt$tf))
...
allData_dt$cond <- interaction(allData_dt$sameTAD, allData_dt$tf)

allData_dt$cond <- factor(allData_dt$cond, levels = c( "sameTAD.shared TF", "sameTAD.no shared TF",   "diffTAD.shared TF", "diffTAD.no shared TF"))
...
subTit <- paste0("max dist. = ", maxDist/1000, " kb - ", dsIn)
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_TAD_TF_bboxplot.", plotType))
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_TAD_TF_boxplot_jitter.", plotType))
ggsave(plot = p_coexpr_boxplot_jitter, filename = outFile, height=myHeightGG, width = myWidthGG*1.5)
cat(paste0("... written: ", outFile, "\n"))


outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_TAD_TF_countPairs_dt.txt"))
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_TAD_TF_countPairs_barplot.", plotType))
