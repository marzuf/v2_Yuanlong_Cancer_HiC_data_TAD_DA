...
# Rscript signif_genes_signif_TADs_v2.R   # 

hicds="K562_40kb"
exprds="TCGAlaml_wt_mutFLT3"

script_name <- "signif_genes_signif_TADs_v2.R"
...
outFolder <- "SIGNIF_GENES_SIGNIF_TADS_v2"
...
for(signif_gene in signif_gene_vars) {
  ...
  for(signif_tad in signif_tad_vars) {
    ...
    nSignif_dt <- nSignif_dt[order(nSignif_dt[,paste0(bothsignifcol)], decreasing = TRUE),]
