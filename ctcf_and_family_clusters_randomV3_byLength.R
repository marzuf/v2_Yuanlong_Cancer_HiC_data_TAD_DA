...
# Rscript ctcf_and_family_clusters_randomV3_byLength.R
...
outFolder <- file.path("CTCF_AND_FAMILY_CLUSTERS_RANDOMV3_BYLENGTH", paste0("ctcf", ctcf_clustSize, "_family", family_clustSize))
...
outFile <- file.path(outFolder, "all_genes_clust_dt.Rdata")
save(all_genes_clust_dt, file=outFile, version=2)
cat(paste0("... written: ", outFile, "\n"))
# load("CTCF_AND_FAMILY_CLUSTERS_RANDOMV3_BYLENGTH/ctcf4000_family260000/all_genes_clust_dt.Rdata")
...
cat(paste0("... written: ", outFile, "\n"))
#  load("CTCF_AND_FAMILY_CLUSTERS_RANDOMV3_BYLENGTH/ctcf4000_family260000/all_fams_agg_cluster_dt.Rdata")

nrow(all_fams_agg_cluster_dt) # 12441
sum(all_fams_agg_cluster_dt$clusterSize > 1) # 1374
...
# how close in length:
all_genes_clust_dt$clusterLength <- all_genes_clust_dt$maxPos - all_genes_clust_dt$minPos + 1
final_fams_cluster_dt$clusterLength <- final_fams_cluster_dt$maxPos - final_fams_cluster_dt$minPos + 1

all_minDiffLength <- sapply(final_fams_cluster_dt$clusterLength, function(x) min(abs(x- all_genes_clust_dt$clusterLength)))
summary(all_minDiffLength)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.0   122.0   363.5   613.1   849.8  7189.0 
              ...
              obsAndPermut_closest_clusters_dt <- foreach(i = 1:nrow(final_fams_cluster_dt)) %dopar% {
                ...
                # random byLength -> match the gene cluster not by # of genes but by cluster length
                # find closest in length all genes cluster
                # v3 => sample among the min values instead of taking the first matching (which.min)
                
                curr_length <- final_fams_cluster_dt$clusterLength[i]
                i_sameLengthV0 <- which.min(curr_length-all_genes_clust_dt$clusterLength)
                allDiff <- curr_length-all_genes_clust_dt$clusterLength
                whichSmallestDiff <- which(allDiff == min(allDiff))
                stopifnot(i_sameLengthV0 %in% whichSmallestDiff)
                i_sameLength <- sample(whichSmallestDiff, size=1)
                ...
                # i_sameSize <- sample(whichSmallestDiff, size=1)
                  ...
                  randomStartPos <- all_genes_clust_dt$minPos[i_sameLength]
                  randomEndPos <- all_genes_clust_dt$maxPos[i_sameLength]
