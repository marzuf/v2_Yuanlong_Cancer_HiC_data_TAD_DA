...
# Rscript revision_changes_cptmtLabels_allDS_allTADs.R

### here keep all TADs for a given hicds, not only those used in the pipeline
...
outFolder <- "REVISION_CHANGES_CPTMTLABELS_ALLDS_ALLTADS"
...
if(buildTable) {
  tad2cptmt_final_dt <- foreach(hicds = names(cptmt_files), .combine='rbind')%do% {
    ...
    ### changed here -> do not substract from final dt but iterate from assigned regions
    ...
    stopifnot(!duplicated(region_dt$region))
    
    hicds_final_dt <- region_dt
...
tad2cptmt_final_dt$region_ID <- file.path(tad2cptmt_final_dt$hicds, tad2cptmt_final_dt$region)
