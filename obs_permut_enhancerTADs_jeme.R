...
script_name <- "obs_permut_enhancerTADs_jeme.R"

# Rscript obs_permut_enhancerTADs_jeme.R lasso
# Rscript obs_permut_enhancerTADs_jeme.R elasticnet
# Rscript obs_permut_enhancerTADs_jeme.R <lasso|elasticnet>

jemeType="lasso"
args <- commandArgs(trailingOnly = TRUE)
stopifnot(lengths(args) == 1)
jemeType <- args[1]
stopifnot(args %in% c("lasso", "elasticnet"))
...
outFolder <- file.path("OBS_PERMUT_ENHANCERTADS_JEME", jemeType)
...
all_hicds <- c("ENCSR489OCU_NCI-H460_40kb" , "ENCSR489OCU_NCI-H460_RANDOMMIDPOS_40kb" )

enh2entrez_dt <- get(load(file.path("JEME_DATA", paste0("jeme_", jemeType, "_dt.Rdata"))))
...
if(buildData) {
  ...
  all_data_dt <- foreach(hicds = all_hicds, .combine='rbind') %dopar%{
    ...
    enh2tad_dt <- get(load(file.path("JEME_DATA", jemeType, paste0(hicds, "_enhancer2tad_dt.Rdata"))))
    #JEME_DATA/elasticnet/ENCSR489OCU_NCI-H460_RANDOMNBRGENES_40kb_enhancer2tad_dt.Rdata
