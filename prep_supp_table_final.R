# Rscript prep_supp_table_final.R

outFolder <- "PREP_SUPP_TABLE_FINAL"
...
dt3 <- get(load("../MANUSCRIPT_FIGURES/FIG_1/DOUBLE_BARPLOT_V2/fig1D_aucRatio_dt.Rdata"))
dt3$dataset <- file.path(dt3$hicds, dt3$exprds)

stopifnot(setequal(dt1$dataset, dt3$dataset))
out1 <- merge(merge(dt1, dt2,by="dataset", all =TRUE), dt3[,c("dataset", "fcc_auc")], by="dataset", all =TRUE)
...
colnames(out1)[colnames(out1) == "fcc_auc"] <- "FCC_AUC"

out1 <- out1[, c("hicds_lab","exprds_lab","nSamp1","nSamp2","FCC_AUC", "nTot", "nSignif","nPurityFlagged","nSignifAndFlagged")]
# stopifnot(!is.na(out1))
out1 <- out1[order(-out1$FCC_AUC,-out1$nSignif, out1$hicds_lab, out1$exprds_lab),]

stopifnot(na.omit(out1)$nSignif >= na.omit(out1)$nSignifAndFlagged )
stopifnot(na.omit(out1)$nPurityFlagged >= na.omit(out1)$nSignifAndFlagged )

outFile <- file.path(outFolder, "suppTable1.txt")
...
outFile <- file.path(outFolder, "suppTable2.txt")
dt5 <- dt5[,c("log10_pval", "Description")]
