...
# Rscript sameFamInSameTAD_hicds_contig_disc1_v2.R  # RUN ELECTRON
...
# not used in disc1 -> discard if 1 single family
...
outFolder <- file.path("SAMEFAMINSAMETAD_HICDS_CONTIG_DISC1_V2_TMP")
          ...
          if(length(contig_tosample) == 0) {
            ...
            for(k in 1:length(contig_tosample)) {
              ...
              while(TRUE) {
                ...
                # changed for v2
                # disc1 -> discard if same family !
                if(any(fams_ratio == 1)) stopifnot(length(unique(samp_fams)) == 1)
                if(any(fams_ratio == 1)) stopifnot(length(fams_ratio) == 1)
