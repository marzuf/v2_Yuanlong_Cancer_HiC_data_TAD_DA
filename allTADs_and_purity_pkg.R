...
# Rscript allTADs_and_purity_pkg.R CPE	
# Rscript allTADs_and_purity_pkg.R ESTIMATE
...
purity_dt <- get(load("Tumor.purity.Rdata"))
purity_dt <- Tumor.purity

stopifnot(pm %in% colnames(purity_dt))

purity_dt$Sample.ID <- substr(purity_dt$Sample.ID, start=1, stop=15)
purity_dt <- purity_dt[,c("Sample.ID", pm)]
purity_dt <- na.omit(purity_dt)
purity_dt <- aggregate(.~Sample.ID, data=purity_dt,FUN=aggregFun)

outFolder <- file.path("ALLTADS_AND_PURITY_PKG", purity_ds, pm, transfExpr)
