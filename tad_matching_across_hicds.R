...
all_matching_dt$overlapGenes_ratio[is.na(all_matching_dt$overlapGenes_ratio)] <- 0

stopifnot(all_matching_dt$overlapBp_ratio >= 0 & all_matching_dt$overlapBp_ratio <= 1)
stopifnot(all_matching_dt$overlapGenes_ratio >= 0 & all_matching_dt$overlapGenes_ratio <= 1)
# 
...
plot_multiDens(list(
  ...
  overlapGenes_ratio = all_matching_dt$overlapGenes_ratio
  ),
...
all_overlapTypes <- c("overlapBp", "overlapGenes")
...
for(overlapType in all_overlapTypes){
  ...
  all_matching_dt_onlyRef <- all_matching_onlyBest_dt[,c("ref_dataset", "refID")]
  ...
  agg_matching_nbr_dt <- aggregate(overlapBp_ratio ~ ref_dataset + matching_dataset , 
                                   data=all_matching_onlyBest_dt, FUN=length)
  colnames(agg_matching_nbr_dt)[colnames(agg_matching_nbr_dt) == past0(overlapType, "_ratio")] <- "nBestMatchs"
  
  
  agg_matching_avgRatio_dt <- aggregate(overlapBp_ratio ~ ref_dataset + matching_dataset , 
