...
# Rscript cmp_same_conditions_v3.R 

script_name <- "cmp_same_conditions_v3.R"
...
outFolder <- "CMP_SAME_CONDITIONS_V3"
...
merge_dt <- all_conds_dt

nbrExprds = length(unique(all_conds_dt$exprds))

nbrCmps <- length(unique(file.path(all_conds_dt$exprds, all_conds_dt$hicds_hicds1, all_conds_dt$hicds_hicds2)))
  ...
  for(plot_var in c("tad_rank", "tad_adjCombPval")) {
    ...
    # outFile <- file.path(outFolder, paste0("all_ds_hicds1_vs_hicds2_", plot_var, ".", plotType))
    # do.call(plotType, list(outFile, height=myHeight, width=myWidth))
    outFile <- file.path(outFolder, paste0("all_ds_hicds1_vs_hicds2_", plot_var, ".", "png"))
    ...
    densplot(
      ...
      # ylab = paste0(gsub("_40kb", "", hicds2)),
      # sub = paste0(hicds2, " vs. ", hicds1),
      main = paste0(plot_var, " - all DS")
    )
    mtext(side=3, text = paste0("# exprds = ",nbrExprds,  " - # cmps = ", nbrCmps), font=3)
    curve(1*x, add=TRUE, lty=2, col="black", lwd=1.5)
    addCorr(
      x = var1, y = var2,
      legPos = "topleft", bty="n"
