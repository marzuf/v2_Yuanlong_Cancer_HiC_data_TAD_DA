...
agg_merged_values_dt <- merge(agg_dt_0[,c("dataset", "region", "purityCorr")],
                              agg_dt_0[,c("dataset", "region", "purityCorr")],
                          by=c("dataset", "region"), suffixes=c("_v0", "_v1"))

outFile <- file.path(outFolder, "cmp_tadLevelCorr_purity_corrected_data.svg")
svg(outFile, height=6, width=6)
...
outFile <- file.path(outFolder, "cmp_geneLevelCorr_purity_corrected_data.svg")
svg(outFile, height=6, width=6)
