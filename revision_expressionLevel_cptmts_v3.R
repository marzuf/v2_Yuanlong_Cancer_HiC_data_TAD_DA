...
# Rscript revision_expressionLevel_cptmts_v3.R

outFolder <- file.path("REVISION_EXPRESSIONLEVEL_CPTMTS_V3")
dir.create(outFolder, recursive=TRUE)

# v3; add mean purity corr
#### #### #### #### #### #### #### #### #### #### #### #### 
...
purityCorr_values <- setNames(merge_dt$purityCorr, merge_dt$region_ID)
...
tad2cptmt_dt <- tad2cptmt_dt[tad2cptmt_dt$region_ID %in% names(purityCorr_values),]
stopifnot(nrow(tad2cptmt_dt) > 0)
tad2cptmt_dt[,paste0("meanPurityCorr")] <- purityCorr_values[tad2cptmt_dt$region_ID]
stopifnot(!is.na(tad2cptmt_dt[,paste0("meanPurityCorr")] ))
...
all_plot_vars <- c("meanPurityCorr", expr_var, "meanCorr", "meanLogFC", "sizeNgenes", "sizeBp")
