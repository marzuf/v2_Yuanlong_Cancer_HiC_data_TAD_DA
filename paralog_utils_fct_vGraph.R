...
get_paralog_concord_with_tad <- function(dt_para, dt_g2t, check_entrez=NULL) {
  
  
  paralog_g2t_dt_tmp <- inner_join(dt_para, dt_g2t[,c("entrezID", "region")], by = c("EntrezGene.ID" = "entrezID"))
  colnames(paralog_g2t_dt_tmp)[colnames(paralog_g2t_dt_tmp) == "region"] <- "ref_region"
  paralog_g2t_dt <- inner_join(paralog_g2t_dt_tmp, dt_g2t[,c("entrezID", "region")], by = c("Human.paralogue.entrezGene.ID" = "entrezID"))
  colnames(paralog_g2t_dt)[colnames(paralog_g2t_dt) == "region"] <- "paralog_region"
  nrow(paralog_g2t_dt)
  # 158274
  stopifnot(is.character(paralog_g2t_dt$EntrezGene.ID))
  stopifnot(is.character(paralog_g2t_dt$Human.paralogue.entrezGene.ID))
  
  if(!is.null(check_entrez))
    stopifnot(setequal(check_entrez, unique(c(paralog_g2t_dt$EntrezGene.ID, paralog_g2t_dt$Human.paralogue.entrezGene.ID))))
  ...
  paralogs_list_nTads <- lengths(paralogs_list_tads)
  
  paralogs_list_ratioTadsGenes <- paralogs_list_nTads/paralogs_list_nGenes
  
  stopifnot(length(paralogs_list_nGenes) == length(paralogs_list_nTads))
