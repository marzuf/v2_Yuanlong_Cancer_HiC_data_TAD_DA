startTime <- Sys.time()
cat(paste0("> Rscript AUC_coexprDist_family_boxplot_permut.R\n"))

# Rscript AUC_coexprDist_family_boxplot_permut.R ENCSR079VIJ_G401_40kb TCGAkich_norm_kich
# Rscript AUC_coexprDist_family_boxplot_permut.R ENCSR489OCU_NCI-H460_40kb TCGAluad_norm_luad
...
outFolder <- file.path("AUC_COEXPRDIST_FAMILY_BOXPLOT_PERMUT", hicds, exprds)
dir.create(outFolder, recursive = TRUE)

familyData <- "hgnc"
familySubData <- "hgnc_family_short"

inFolder <- file.path("AUC_COEXPRDIST_WITHFAM_SORTNODUP_PERMUT")
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_TAD_sameFam_boxplot.", plotType))
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_TAD_sameFam_boxplot_jitter.", plotType))
ggsave(plot = p_coexpr_boxplot_jitter, filename = outFile, height=myHeightGG, width = myWidthGG*1.5)
cat(paste0("... written: ", outFile, "\n"))


outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_TAD_sameFam_countPairs_dt.txt"))
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_TAD_sameFam_countPairs_barplot.", plotType))
