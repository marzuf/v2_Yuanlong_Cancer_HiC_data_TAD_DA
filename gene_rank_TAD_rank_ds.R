...
# Rscript gene_rank_TAD_rank_ds.R LG1_40kb TCGAlusc_norm_lusc
...
script_name <- "gene_rank_TAD_rank_ds.R"
...
all_hicds <- hicds
#all_exprds <- file.path(pipFolder, hicds, exprds)
all_exprds <- exprds
...
outFolder <- file.path("GENE_RANK_TAD_RANK_DS")
...
file_suffix <- paste0("tad_pval_thresh", tad_pval_thresh, "_gene_pval_thresh", gene_pval_thresh, "_", hicds, "_", exprds)
...
if(buildTable) {
  ...
  outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_all_gene_tad_signif_dt.Rdata"))
...
for(xvar in all_x) {
  for(yvar in all_y) {
    ...
    outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_", yvar, "_vs_", xvar, "_densplot.", plotType))    
    ...
    mtext(side=3, text=paste0(hicds, " - ", exprds, "; n = ", nrow(all_gene_tad_signif_dt)))
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_", yvar, "_vs_", xvar, "_withThresh_densplot.", plotType))    
do.call(plotType, list(outFile, height=myHeight, width=myWidth))
plot(
  x = myx,
  y = myy,
...
for(xvar in all_x) {
  for(yvar in all_y) {
    ...
    outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_", yvar, "_vs_", xvar, "_subMissed_densplot.", plotType))    
    do.call(plotType, list(outFile, height=myHeight, width=myWidth))
    densplot(
      x = myx,
      y = myy,
    ...
    mtext(side=3, text=paste0(hicds, " - ", exprds, "; n = ", nrow(limmaMissed_dt)))
    addCorr(x=myx, y=myy, legPos = "topleft", bty="n")
    foo <- dev.off()
    cat(paste0("... written: ", outFile, "\n"))  
  }  
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_", yvar, "_vs_", xvar, "_subMissed_label_scatter.", plotType))    
do.call(plotType, list(outFile, height=myHeight, width=myWidth))
plot(
  x = myx,
  y = myy,
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_", yvar, "_vs_", xvar, "_subMissed_label_scatter_sameFCsign.", plotType))    
do.call(plotType, list(outFile, height=myHeight, width=myWidth))
plot(
  x = myx,
  y = myy,
...
for(ct in all_cmpTypes) {
  ...
  if(nrow(ct_limmaMissed_dt) == 0) next
...
outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_nbr_limma_missed_genes_byTAD_boxplot.", plotType))
