...
# Rscript smilePlot_random.R

script_name <- "smilePlot_random.R"
...
all_patterns <- c("RANDOMMIDPOSDISC", "RANDOMMIDPOSSTRICT", "RANDOMMIDPOS") # have deleted the permdt for randommidpos...
all_patterns <- c("RANDOMMIDPOSDISC", "RANDOMMIDPOSSTRICT") # have deleted the permdt for randommidpos...

for(rd_patt in all_patterns) {

  outFolder <- file.path("SMILEPLOT_RANDOM", rd_patt)
  dir.create(outFolder, recursive=TRUE)
  
  
  stopifnot(dir.exists(pipFolder))
  all_hicds <- list.files(pipFolder)
  all_hicds <- all_hicds[grepl(paste0(rd_patt, "_40kb"),  all_hicds)]
