...
foo <- foreach(ds = all_ds) %dopar% {
  ...
  outFolder <- file.path(paste0(folderName, "_",binSizeKb , "kb"), "FINAL_DOMAINS_WITH_SCORES")
  ...
  for(chromo in all_chrs) {
    ...
    chr_DT$CD_rank_values <- as.numeric(as.character(chr_DT$CD_rank))
    ...
    tadPos_DT <- do.call(rbind, by(all_bins_tadPos_DT, all_bins_tadPos_DT$domainNbr, function(x) {
      ...
      data.frame(
        ...
        rankValues = unique(x$CD_rank_values),
    ...
    outDT <- tadPos_DT[,c("chr", "start", "end", "rankValues"), drop=FALSE]
    stopifnot(is.numeric(outDT$start))
    stopifnot(is.numeric(outDT$end))
    stopifnot(is.numeric(outDT$rankValues))
    ...
    outFile <- file.path(outFolder, paste0(folderName, "_", chromo, "_", "YL", "_", binSizeKb, "kb_final_domains_with_scores.txt"))
