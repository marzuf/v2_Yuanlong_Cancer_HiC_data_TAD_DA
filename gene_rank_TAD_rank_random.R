...
# Rscript gene_rank_TAD_rank_random.R

hicds="K562_40kb"
exprds="TCGAlaml_wt_mutFLT3"

script_name <- "gene_rank_TAD_rank_random.R"
...
all_hicds_start <- all_hicds

for(rd_patt in all_patterns) {

  
  all_hicds_init <- all_hicds_start[! (grepl("PERMUT", all_hicds_start) | grepl("RANDOM", all_hicds_start))]
  all_hicds <- all_hicds_start[ grepl(paste0(rd_patt, "_40kb"), all_hicds_start) ]
  stopifnot(length(all_hicds) == length(all_hicds_init))  
  ...
  stopifnot(length(unique(file.path(final_dt$hicds, final_dt$exprds))) == length(all_datasets))
  
  outFolder <- file.path("GENE_RANK_TAD_RANK_RANDOM", rd_patt)
