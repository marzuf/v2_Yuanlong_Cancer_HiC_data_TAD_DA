...
# Rscript tad_matching_notSignif_across_hicds_allMatch_v2.R
# Rscript tad_matching_notSignif_across_hicds_allMatch_v2.R norm_vs_tumor
# Rscript tad_matching_notSignif_across_hicds_allMatch_v2.R subtypes
# Rscript tad_matching_notSignif_across_hicds_allMatch_v2.R wt_vs_mut
...
outFolder <- file.path("TAD_MATCHING_NOTSIGNIF_ACROSS_HICDS_ALLMATCH_v2", data_cmpType)
dir.create(outFolder, recursive = TRUE)

logFile <- file.path(outFolder, "tad_matching_notSignif_across_hicds_logFile.txt")
file.remove(logFile)
...
final_dt[, paste0(signifcol)] <- final_dt[, paste0(signif_column)] > signifThresh  ## => changed here, select the not signif
...
notSignif_tads <- file.path(final_dt$hicds[final_dt[, paste0(signifcol)] ],
                         final_dt$exprds[final_dt[, paste0(signifcol)] ],
                         final_dt$region[final_dt[, paste0(signifcol)] ])

outFile <- file.path(outFolder, paste0(file_prefix, "notSignif_tads", signif_column, signifThresh, "_minBpRatio", minOverlapBpRatio, "_minInterGenes", minIntersectGenes, ".Rdata"))
save(notSignif_tads, file=outFile, version=2)
...
if(buildTable) {
  ...
  all_data_list <- foreach(hicds = all_hicds) %dopar% {
    ...
    exprds_list <- foreach(exprds = all_exprds[[paste0(hicds)]]) %do% {
      ...
      signif_ref_g2t_dt <- ref_g2t_dt[ref_g2t_dt$region %in% notSignif_tads,]
      signif_geneList <- geneList[geneList %in% as.character(signif_ref_g2t_dt$entrezID)]
      
      signif_ref_tadpos_dt <- ref_tadpos_dt[ref_tadpos_dt$region %in% notSignif_tads,]
  ...
  all_notSignif_matching_dt <- foreach(ref_dataset = all_datasets, .combine='rbind') %dopar% {
  ...
  outFile <- file.path(outFolder, paste0(file_prefix, "all_notSignif_matching_dt_", signifcol, ".Rdata"))
  save(all_notSignif_matching_dt, file=outFile, version=2)  
  cat(paste0("... written: ", outFile, "\n"))
  
} else {
  outFile <- file.path(outFolder, paste0(file_prefix, "all_notSignif_matching_dt_", signifcol, ".Rdata"))
  cat("... load data\n")
  all_notSignif_matching_dt <- get(load(outFile))
  outFile <- file.path(outFolder, "all_data_list.Rdata")
  all_data_list <- get(load(outFile))
}
all_notSignif_matching_dt$refID_full <- file.path(all_notSignif_matching_dt$ref_dataset, all_notSignif_matching_dt$refID)
all_notSignif_matching_dt$queryID_full <- file.path(all_notSignif_matching_dt$query_dataset, all_notSignif_matching_dt$queryID)
...
txt <- paste0("> FILTER 1 - # of match\t=\t", nrow(all_notSignif_matching_dt), "\n")
cat(txt)
cat(txt, file=logFile, append=T)
all_notSignif_matching_dt  <- all_notSignif_matching_dt[all_notSignif_matching_dt$overlapBpRatio >= minOverlapBpRatio,]
txt <- paste0("> FILTER 1 - # of match with overlap bp >= ", minOverlapBpRatio, "\t=\t", nrow(all_notSignif_matching_dt), "\n")
cat(txt)
cat(txt, file=logFile, append=T)

all_notSignif_tads_with_match <- unique(all_notSignif_matching_dt$refID_full)
length(all_notSignif_tads_with_match) # 
sum(final_dt[,paste0(signifcol)]) # 

tad=all_notSignif_tads_with_match[1]
set_of_tads <- foreach(tad = all_notSignif_tads_with_match) %dopar% {
  # all_notSignif_matching_dt[all_notSignif_matching_dt$refID_full == tad,]
  matching_tads <- all_notSignif_matching_dt$queryID_full[all_notSignif_matching_dt$refID_full == tad]
  sort(c(tad, matching_tads))
}
names(set_of_tads) <- all_notSignif_tads_with_match
...
conserved_notSignif_tads <- set_of_tads
outFile <- file.path(outFolder, paste0(file_prefix, "conserved_notSignif_tads", signif_column, signifThresh, "_minBpRatio", minOverlapBpRatio, "_minInterGenes", minIntersectGenes, ".Rdata"))
save(conserved_notSignif_tads, file=outFile, version=2)
cat(paste0("... written: ", outFile, "\n"))
...
outfile <- file.path(outFolder, paste0(file_prefix, "tad_occurences_in_conserved_notSignif_tads", signif_column, signifThresh, "_minBpRatio", minOverlapBpRatio, "_minInterGenes", minIntersectGenes, ".",plotType))
...
#######################################################################################################################################                                       
all_datasets <- unique(all_notSignif_matching_dt$ref_dataset)
...
outFile <- file.path(outFolder, paste0(file_prefix, "conserved_regions_with_genes_notSignif_tads", signif_column, signifThresh, "_minBpRatio", minOverlapBpRatio, "_minInterGenes", minIntersectGenes, "_df.txt"))
...
outFile <- file.path(outFolder, paste0(file_prefix, "_", cmpType, "_conserved_regions_with_genes_notSignif_tads", signif_column, signifThresh, "_minBpRatio", minOverlapBpRatio, "_minInterGenes", minIntersectGenes, "_df.txt"))
