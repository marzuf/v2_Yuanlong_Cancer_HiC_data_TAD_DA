...
# v1 normalization: scale() zscore transf.

# CORRECTED: before normalization and mean, add the 0 values in the count vector !

# Rscript revision_inter_intra_proba_corrected_pairedHiC.R
script_name="revision_inter_intra_proba_corrected_pairedHiC.R"
startTime <- Sys.time()
...
tmp_pairs <- c(
  file.path("LI_40kb","GSE105381_HepG2_40kb"),
  file.path("LG1_40kb" ,"ENCSR444WCZ_A549_40kb"),
  file.path("LG2_40kb" ,"ENCSR444WCZ_A549_40kb" ),
  file.path("LG1_40kb", "ENCSR489OCU_NCI-H460_40kb"), 
  file.path("LG2_40kb", "ENCSR489OCU_NCI-H460_40kb"), 
  file.path("GSE118514_RWPE1_40kb", "ENCSR346DCU_LNCaP_40kb"),
  file.path("GSE118514_RWPE1_40kb", "GSE118514_22Rv1_40kb")
)
...
outFolder <- "REVISION_INTER_INTRA_PROBA_CORRECTED_PAIREDHIC"
...
if(buildTable) {
  all_inter_intra_dt <- foreach(i_ds = seq_along(all_pairs), .combine='rbind') %do% {
    ...
    all_chromo_dt <- foreach(chromo = all_chrs,.combine='rbind') %do% {
      ...
      # stopifnot(!is.na(matched_matNorm_dt$normCount)) # not true for v1 if 1 signle value
