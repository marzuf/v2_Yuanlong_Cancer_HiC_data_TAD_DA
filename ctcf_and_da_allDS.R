...
tad_nbrSing_dt <- aggregate(nInClust ~ hicds + exprds + region+meanLogFC+meanCorr+adjPvalComb, data =clust_merged_dt, FUN=function(x) sum(x == 1))
...
nSingleton_breaks <- c(0,5,6,9,15)
# cat(paste0("... running get_fract_lab2\n"))
# check_labs <- get_fract_lab2(vect_values=tad_conv_dt$nSingleton, range_levels = nSingleton_breaks)
# stopifnot(gsub("<=0", "0",check_labs) == tad_conv_dt$nSingleton_lab)
# cat(paste0("... running get_fract_lab0\n"))
# check_labs <- get_fract_lab0(vect_values=tad_conv_dt$nSingleton, range_levels = nSingleton_breaks)
# stopifnot(gsub("<=0", "0",check_labs) == tad_conv_dt$nSingleton_lab)
