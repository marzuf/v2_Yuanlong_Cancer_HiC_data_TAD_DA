...
# Rscript revision_interactome_withSignif_ratio_v2.R
...
outFolder <- file.path("REVISION_INTERACTOME_WITHSIGNIF_RATIO_V2")
...
if(buildTable){
  ...
  for(ds in names(all_pairs_cols)) {
    ...
    dt <- get(load(file.path(setDir,
                             ...
                             paste0(ds, "_TADs_for_hicdc_withSignif.output.v2.Rdata"))))
...
all_plot_dt <- foreach(cmp = all_cmps, .combine='rbind') %do% {
  ...
  for(icol in densplotcols) {
    ...
    if(icol == "p_two_side") {
      ...
      # sub_plot_dt$match_ref_ratio <- as.numeric(sub_plot_dt[,paste0(icol,"_", match_hicds_col  )])
      # sub_plot_dt$match_ref_ratio <- as.numeric(sub_plot_dt[,paste0(myicol  )])
      sub_plot_dt$match_ref_ratio <- -log10(as.numeric(sub_plot_dt[,paste0(myicol  )]))
      
      tmp_dt <- sub_plot_dt
      tmp_dt$interactome_signif <- tmp_dt$match_ref_ratio >= -log10(interactome_signif)
      ...
      # cat(paste0(paste0(icol,"_",ref_hicds_col, "_", match_hicds_col  ), "\n"))
      # cat(paste0("..\n",colnames(sub_plot_dt), "\n"))
      sub_plot_dt$match_ref_ratio <- sub_plot_dt[,paste0(icol,"_",ref_hicds_col, "_", match_hicds_col  )]
    } else {
      ...
      sub_plot_dt$match_ref_ratio <- sub_plot_dt[,paste0(icol, "_", match_hicds_col)] / 
    ...
    myy <- -log10(sub_plot_dt$adjPvalComb)
    myx <- sub_plot_dt$match_ref_ratio
    ...
    sub_plot_dt[,paste0(icol, "_match_ref_ratio")] <- sub_plot_dt$match_ref_ratio
  } # end iterating over columns to plot
  sub_plot_dt$ref_hicds <- ref_hicds
  sub_plot_dt$match_hicds <- match_hicds
  sub_plot_dt[,c("ref_hicds","exprds","adjPvalComb", "match_hicds","tad_signif",paste0(densplotcols, "_match_ref_ratio"), densitycols)]
} # end iterating over ds
...
for(icol in densplotcols) {
  ...
  plotcol <- paste0(icol, "_match_ref_ratio")
