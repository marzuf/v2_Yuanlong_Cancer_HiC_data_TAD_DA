...
# Rscript revision_GO_tft_signif.R mir
# Rscript revision_GO_tft_signif.R tft
# Rscript revision_GO_tft_signif.R all
...
script_name <- "revision_GO_tft_signif.R"
...
#   gmtFile <- file.path(setDir, "/mnt/ed4/marie/scripts/TAD_DE_pipeline_v2", 
#                        paste0("c5.", tolower(enricher_ontologyType), ".v6.1.entrez.gmt"))
# } else {
...
tf_type <- "tft"
tf_type <- "all"
tf_type <- commandArgs(trailingOnly = TRUE)[1]
stopifnot(tf_type %in% c("mir", "tft", "all"))

gmtFile <- paste0("c3.", tf_type, ".v7.2.entrez.xls")

stopifnot(file.exists(gmtFile))
c5_msigdb <- read.gmt(gmtFile)


outFolder <- file.path("REVISION_GO_TFT_SIGNIF", toupper(tf_type))
