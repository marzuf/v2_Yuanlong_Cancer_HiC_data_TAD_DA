startTime <- Sys.time()
cat(paste0("> Rscript coexprDist_diffTAD_boxplot_cmp_permut.R\n"))

# Rscript coexprDist_diffTAD_boxplot_cmp_permut.R
# Rscript coexprDist_diffTAD_boxplot_cmp_permut.R ENCSR489OCU_NCI-H460_40kb TCGAluad_norm_luad
...
FOO <- foreach(hicds = all_hicds, .combine='rbind') %dopar% {
  ...
  FOO <- foreach(exprds = all_exprds[[paste0(hicds)]], .combine='rbind') %do% {
    ...
    outFolder <- file.path("COEXPRDIST_DIFFTAD_BOXPLOT_CMP_PERMUT", hicds, exprds)
    ...
    to_merge_dt <- allData_dt[ allData_dt$sameTAD == "diffTAD",]
    ...
    stopifnot(merged_dt$sameTAD == "diffTAD")
    ...
    subTit <- paste0("diffTAD gene pairs; max dist. = ", maxDist/1000, " kb")
    ...
    outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_cmp_obs_permut_diffTAD_boxplot.", plotType))
    ...
    outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_cmp_obs_permut_diffTAD_boxplot_jitter.", plotType))
    ggsave(plot = p_coexpr_boxplot_jitter, filename = outFile, height=myHeightGG, width = myWidthGG*1.5)
    cat(paste0("... written: ", outFile, "\n"))
    
    
    outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_cmp_obs_permut_diffTAD_countPairs_dt.txt"))
    ...
    outFile <- file.path(outFolder, paste0(hicds, "_", exprds, "_coexpr_cmp_obs_permut_diffTAD_countPairs_barplot.", plotType))
