...
for(plot_var in all_plot_vars) {
  ...
  qts <- quantile(na.omit(merge_dt[,paste0(plot_var)]), probs=c(0.05, 0.95))
  ...
  mySub <- paste0("# DS comparisons = ", length(all_cmps), "; # TADs = ", nrow(sub_dt), 
                  " (signif.: ", sum(sub_dt$adjPval <= tadSignifThresh), "); crop ", plot_qt1,"-", plot_qt2, "")
  ...
  plot(x = -log10(merge_dt$adjPvalComb),
       y  = log10(merge_dt$mean_intra))
  
  
  plot(x = -log10(merge_dt$adjPvalComb),
       y  = merge_dt$mean_intraNorm)
