...
# Rscript tad_matching_signif_across_hicds_allMatch_v2_shuffle_purityFilter_final.R
# Rscript tad_matching_signif_across_hicds_allMatch_v2_shuffle_purityFilter_final.R norm_vs_tumor
# Rscript tad_matching_signif_across_hicds_allMatch_v2_shuffle_purityFilter_final.R subtypes
# Rscript tad_matching_signif_across_hicds_allMatch_v2_shuffle_purityFilter_final.R wt_vs_mut
...
script_name <- "tad_matching_signif_across_hicds_allMatch_v2_shuffle_purityFilter_final.R"
...
outFolder <- file.path("TAD_MATCHING_SIGNIF_ACROSS_HICDS_ALLMATCH_v2_SHUFFLE_PURITYFILTER_FINAL", data_cmpType, purity_ds, pm, transfExpr)
...
#    foo_permut <- foreach(i_perm=c(588, 671)) %dopar% {
    ...
    all_data_list <- foreach(hicds = all_hicds) %do% {
      ...
      exprds_list <- foreach(exprds = all_exprds[[paste0(hicds)]]) %do% {
        ...
        nSignif <- length(signif_tads)
        signif_tads <- signif_tads[! signif_tads %in% ds_TADs_to_discard]  # !!! for aran, not all data -> important not the same as ...%in%..._to_keep !!!
        nSignif_filtered <- length(signif_tads)
        ######################################## 
        ...
        list(
          ...
          nSignifFiltered=nSignif_filtered,
          nSignif=nSignif
        )
      }
