# Rscript cmp_familyModules_belowAbove_sameTADratio.R

inFolderSame <- "FAMILYMODULES_RUNTADMEANCORRRATIODOWN_SAMETAD"
inFolderDiff <- "FAMILYMODULES_RUNTADMEANCORRRATIODOWN"
nMaxTADsize <- 1
maxSameTAD <- 0.5
...
outFolder <- file.path("CMP_FAMILYMODULES_BELOWABOVE_SAMETADRATIO", nMaxTADsize)
dir.create(outFolder, recursive = TRUE)

sameTAD_rdCmpnt <- lapply(sameTAD_data, function(sublist) lapply(sublist, function(x) x[["famCmpnt_ratioDown"]]))
diffTAD_rdCmpnt <- lapply(diffTAD_data, function(sublist) lapply(sublist, function(x) x[["famCmpnt_ratioDown"]]))
...
sameTAD_corrCmpnt <- lapply(sameTAD_data, function(sublist) lapply(sublist, function(x) x[["famCmpnt_meanCorr"]]))
diffTAD_corrCmpnt <- lapply(diffTAD_data, function(sublist) lapply(sublist, function(x) x[["famCmpnt_meanCorr"]]))
...
plot_multiDens(
  plot_list,
  plotTit = paste0("TAD meanCorr (family components; max. size=", nMaxTADsize, "*mean TAD size)")
)
mtext(side=3, text=paste0("sameTAD: > ", maxSameTAD, " same TAD; diffTAD: <=", maxSameTAD, " sameTAD"), font=3)
foo <- dev.off()
